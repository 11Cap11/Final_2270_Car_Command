...
City::City(){
  customerQueue = new PriorityQueue(10);
...
double City::serveCustomer(){//This is the process by which we calculate the price for customers and keep track of the car's charge.
    if(customerQueue->isEmpty()){//Sanity Check.
        return 0.0;
    }
    double pricePerDist = 1.25 + (0.25 * (customerQueue->peek().priority));//Calculations for cost based on priority and distance.
    int distance = shortestDistance(customerQueue->peek().startLocation, customerQueue->peek().endLocation);//Calling the shortist distance function.
    double tripPrice = pricePerDist*distance;//We multiply distance by price per distance to find the total.
    if((myCar.chargeDistance+3) <= distance){//When the car runs low on fuel it has to stop to recharge on the way.
        myCar.chargeDistance = 30;//Resetting the car's fuel;
        tripPrice = tripPrice - 2.0;//Discount to customer for their wait.
        cout<<"car requires charging stop, discount applied"<<endl;
    }
    myCar.chargeDistance = myCar.chargeDistance - distance;//Calculates new car charge.
    myCar.nearest = customerQueue->peek().endLocation;//car's location changes.
    customerQueue->dequeue();//Customer has been served, so they are removed from queue.
    cout << "Total Trip Cost: " << tripPrice << endl;
    return tripPrice;
};

void City::printCustomers(){
